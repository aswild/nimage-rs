/*!
 * Basic CRC32 implementation ported from xcrc32 of the libiberty library.
 * Always uses 0 as a starting value, as used by the nImage format.
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 * Copyright (C) 2009-2018 Free Software Foundation, Inc.
 * Copyright (C) 2020 Allen Wild (rust port)
 *
 * This CRC can be specified as:
 *
 *   Width  : 32
 *   Poly   : 0x04c11db7 (IEEE polynomial)
 *   Init   : 0
 *   RefIn  : false
 *   RefOut : false
 *   XorOut : 0
...
/**
 * Main struct type for a CRC32 accumulator. It's really just a wrapped u32.
 */
pub struct Crc32 {
    crc: u32,
}
...
impl Crc32 {
    ...
    /**
     * Add some more data to the CRC.
     */
    pub fn write(&mut self, buf: &[u8]) {
        self.crc = buf.iter().fold(self.crc, |acc: u32, b: &u8| {
     ...
     * This doesn't "finalize" the Crc32 object, more data can be added later.
     */
    pub fn sum(&self) -> u32 {
        self.crc
    }
...
impl<R: io::Read> Reader<R> {
    ...
    /**
     * Get the CRC32 of all data read so far.
     */
    pub fn sum(&self) -> u32 {
        self.crc.sum()
    }
...
impl<R: io::Read> io::Read for Reader<R> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        // that no bytes were read" because the CRC write can never fail.
        let ret = self.inner.read(buf);
        if let Ok(count) = ret {
            self.crc.write(&buf[..count]);
        }
        ret
    }
}
...
impl<W: io::Write> Writer<W> {
    ...
    /**
     * Get the CRC32 of all data written so far.
     */
    pub fn sum(&self) -> u32 {
        self.crc.sum()
    }
